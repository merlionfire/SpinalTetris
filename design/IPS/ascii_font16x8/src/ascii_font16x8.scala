...
class ascii_font16x8 ( addressWidth : Int = 11, wordWidth : Int = 8) extends BlackBox {
  ...
  val io = new Bundle {
    val clk = in Bool()
    val font_bitmap_addr = in UInt( addressWidth bits)
    val font_bitmap_byte = out Bits(  wordWidth bits )
  }
  // Map the clk
...
/*
  ...
  //  Pro : concise
  //      : function setRtlInLine() is duplicated by any blackbox components.
  addRTLPath("rtl/ascii_font16x8.v")
  ...
  setRtlInLine()
*/
  // Method 2 : Copy all contents here directly.
/*
  6K-bit Data and 2K-bit Parity Single-Port Synchronous Block RAM with 9-bit Port
  setInlineVerilog {
    s"""
     |RAMB16_S9 #(
     |  .INIT    ( 9'h000 ),
